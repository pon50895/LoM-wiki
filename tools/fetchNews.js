...
async function main() {
    ...
    const [rssRes, apiRes] = await Promise.all([axios.get(RSS_URL), axios.get(API_URL)]);
    const byDate = new Map();
    for (const n of apiRes.data.appnews.newsitems) {
    ...
    const items = parseRss(rssRes.data).filter(i => i.gid).sort((a, b) => a.date - b.date);
